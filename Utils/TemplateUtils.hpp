...
struct LambdaUtil<ReturnType(ClassType::*)(Types* ...) const>
{
    ...
    template <typename DestinationLambda, typename SourceLambda>
    static void ArgInjectTemplateLambda(DestinationLambda destinationLambda, SourceLambda sourceLambda)
    {
        destinationLambda(sourceLambda.template operator() < Types > ()...);
    }
};
